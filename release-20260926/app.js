...
function applyModuleConfig() {
  ...
  const visibleTravelLinks = [...document.querySelectorAll(".travel-nav-link[data-module]")].filter((link) => !link.hidden);
...
function hubTransfersMarkup(day) {
  const legs = (state.data.hubTransfers || []).filter((leg) => leg.day === day.day);
  if (!legs.length) return "";
  return `<section class="hub-transfer-guide" aria-label="交通枢纽接驳"><h3>枢纽接驳</h3>${legs.map((leg) => `
    <article class="hub-transfer-leg"><h4>${escapeHtml(leg.title)}</h4>
      <p><strong>建议：</strong>${escapeHtml(leg.recommended)}</p>
      ${leg.alternative ? `<p><strong>备选：</strong>${escapeHtml(leg.alternative)}</p>` : ""}
      <div class="hub-transfer-links">${(leg.links || []).map((link) => `<a href="${escapeHtml(safeExternalUrl(link.url))}" target="_blank" rel="noopener noreferrer">${escapeHtml(link.label)} ↗</a>`).join("")}</div>
...
function renderTravelPrep() {
  renderTodoList();
  renderBookingChecklist();
  $("#booking-checklist").onchange = (event) => {
    const checkbox = event.target.closest("[data-booking-ticket]");
    if (!checkbox) return;
    if (checkbox.checked) state.purchasedTickets.add(checkbox.dataset.bookingTicket);
    else state.purchasedTickets.delete(checkbox.dataset.bookingTicket);
    saveTicketState(checkbox.dataset.bookingTicket, checkbox.checked);
    updateInlineTicketState(checkbox.dataset.bookingTicket, checkbox.checked);
  };
...
function renderBookingChecklist() {
  const root = $("#booking-checklist");
  if (!root) return;
  const tickets = state.data.ticketPlanning?.items || [];
  ...
  root.innerHTML = tickets.map((ticket) => {
    ...
    return `<div class="booking-checklist-item${isTicketPurchased(ticket) ? " is-complete" : ""}">
      <label><input type="checkbox" data-booking-ticket="${escapeHtml(ticket.id)}" ${isTicketPurchased(ticket) ? "checked" : ""}>
        <span><strong>${escapeHtml(ticketTitle(ticket))}</strong><small>${escapeHtml(day?.date.slice(5).replace("-", "/") || "")} · ${escapeHtml(ticketRequirement(ticket))}</small></span></label>
      ${url ? `<a href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer">官网预约 ↗</a>` : ""}
    </div>`;
  }).join("");
}
...
function preloadDefaultRouteMap() {
  return; // Geographic tiles load for the visible viewport instead of preloading the old illustration.
...
async function init() {
  try {
    const response = await fetch("release-20260926/trip-data.json", { cache: "no-store" });
